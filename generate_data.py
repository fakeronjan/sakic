...
for rid, pairs in _pairs.items():
    ...
    for i, (team, odds) in enumerate(pairs, start=1):
        if odds != prev_o:
            prev_r, prev_o = i, odds
        rmap[team] = prev_r
    _title_odds_rank_cache[rid] = rmap
...
for s in sorted(ws_results.keys(), reverse=True):
    ...
    champs_list.append({
        ...
        "champion": {
            ...
            "ps_end_rating_o": round(float(ch_ps["rating_o"].iloc[0]), 3) if not ch_ps.empty and not pd.isna(ch_ps["rating_o"].iloc[0]) else None,
            "ps_end_rating_d": round(float(ch_ps["rating_d"].iloc[0]), 3) if not ch_ps.empty and not pd.isna(ch_ps["rating_d"].iloc[0]) else None,
            "ps_end_rank_o":   int(ch_ps["rank_o"].iloc[0]) if not ch_ps.empty and not pd.isna(ch_ps["rank_o"].iloc[0]) else None,
            "ps_end_rank_d":   int(ch_ps["rank_d"].iloc[0]) if not ch_ps.empty and not pd.isna(ch_ps["rank_d"].iloc[0]) else None,
        ...
        "runner_up": {
            ...
            "ps_end_rating_o": round(float(ru_ps["rating_o"].iloc[0]), 3) if not ru_ps.empty and not pd.isna(ru_ps["rating_o"].iloc[0]) else None,
            "ps_end_rating_d": round(float(ru_ps["rating_d"].iloc[0]), 3) if not ru_ps.empty and not pd.isna(ru_ps["rating_d"].iloc[0]) else None,
            "ps_end_rank_o":   int(ru_ps["rank_o"].iloc[0]) if not ru_ps.empty and not pd.isna(ru_ps["rank_o"].iloc[0]) else None,
            "ps_end_rank_d":   int(ru_ps["rank_d"].iloc[0]) if not ru_ps.empty and not pd.isna(ru_ps["rank_d"].iloc[0]) else None,
